...
@export var refresh_rate : float = 0.05
...
var character_data : CharacterAtt
...
func install_bar(_actor_profile:ActorProfile) -> void:
	...
	var char_cur_atts    = _actor_profile.current_cdata
	var char_origin_atts = _actor_profile.original_cdata
	character_data = char_cur_atts
	actor_profile = _actor_profile
	character_data.changed.connect(attribute_updated)
	
	current_value = character_data.get_att(att_name)[1]
	max_value = char_origin_atts.get_att(att_name)[1]
...
func attribute_updated() -> void:
	...
	var val = character_data.get_att(att_name)[1]
